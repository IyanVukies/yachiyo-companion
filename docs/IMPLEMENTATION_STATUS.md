...
| Live2D Mao                 | Externally limited          | Adapter/shaders/protocol built; Core absent, so actual rendering unavailable      |
| RVC                        | Safe fallback               | Model/index verified; runtime, RMVPE, ContentVec absent                           |
...
| Hardening and tests        | Complete                    | Strict TS, lint, formatting, 32 Vitest, 5 pytest, Electron E2E                    |
...
- Cubism Core execution and Mao drawing/expression/motion/physics/blink/`ParamA` cannot be claimed without the official Core file.
- Kobo RVC conversion cannot be claimed without trusted inference dependencies and model licensing.
